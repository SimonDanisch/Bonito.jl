...
function jsrender(session::Session, err::Base.CapturedException)
    text = sprint(showerror, err; context = :color => true)
    return jsrender(session, DOM.div(RichText(text);
        style = Styles("border-left" => "3px solid #e5484d",
                       "padding" => "4px 8px")))
end
