@testset "serialization" begin
    ...
    expect = "console.log('hey'); Dashi.deserialize_js({\"__javascript_type__\":\"TypedVector\",\"payload\":[1.0,2.0]}); Dashi.deserialize_js([2,4]); var hello = 1;"
    @test string(js_str) == expect

    asset = Dashi.Asset("file.dun_exist"; check_isfile=false)
    test_throw() = Dashi.include_asset(Dashi.Asset("file.dun_exist"))
...
@testset "http" begin
    @test_throws ErrorException("Invalid sessionid: lol") Dashi.request_to_sessionid((target="lol",))
    @test Dashi.request_to_sessionid((target="lol",), throw=false) === nothing
end
...
@testset "async messages" begin
    ...
    app = Dashi.Server(handler, "0.0.0.0", 8558)
    try
...
@testset "relocatable" begin
    deps = [
        ...
        Dashi.PakoLib => "js",
        Dashi.DashiLib => "js",
        Dashi.MarkdownCSS => "css",
        Dashi.TailwindCSS => "css",
        Dashi.Styling => "css",
    ]
