...
- `jsrender(session, x)`: session-specific methods first; generic fallback
  ...
  returns a String (valid CHILD, not a Node), so `session_dom(session, ::App)`
...
- `session_dom(session, dom::Node; init, html_document)`: builds the fragment
  ...
  (ROOT ONLY, since subs share the root transport), and when `init=true`, the
...
- `JSDoneLoading`: error variant records on `init_error`; success resolves the
  ...
  async, which is what flips `connection_ready`/flushes the queue via
...
JS side (`Sessions.js`):
- `init_session(id, message_promise|null, status, compression)` registers the
...
- `add_cached!(create, session, send_to_js, object)`: key = `cache_key(session,
  ...
  `TrackingOnly(key)`.
- Proxied roots take the same TrackingOnly branch as any other root, because
  there is one proxied root per browser page (§9): §0 holds, so whatever a
  prior sub shipped is already in that page's cache.
- `evaljs_value` builds a throwaway comm Observable OUTSIDE session lifetime, so
...
- `render_asset(session, server, asset)`: the `<script>`/`<link>` element
  ...
  use `<script src=data:… type=module>` for importable modules, so it evaluates
...
- `render_proxied(app, prefix; driver, asset_server)`: fresh proxied ROOT
  ...
  same bundle as the surrounding DOM, so it works through innerHTML mounts).
...
- Proxied assets: worker `ProxyAssetRegistry` refcounts; 0→1 ships
  ...
  An `AssetEntry` keeps every live registration of a path (the key is a
  content hash, so only the driver differs): the newest is served, and
  releasing one falls back to a survivor rather than stranding the path on a
  closed bridge.
...
- ES6Module bundles: editing an IMPORTED file doesn't bump the entry's mtime, so
